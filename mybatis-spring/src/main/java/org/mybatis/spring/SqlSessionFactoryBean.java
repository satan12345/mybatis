...
public class SqlSessionFactoryBean
  ...
  /**
   * 创建一个SqlSessionFactoryBuilder的构建器采用的设计模式就是 建造者模式
   */
  ...
  /**
   * mybatis的插件拦截器对象 Executor (update, query, flushStatements, commit, rollback, getTransaction, close, isClosed)
  ...
  /**
   * mybatis的类型处理器:作用用于转化java类型 和数据库类型
   */
  ...
  /**
   * 别名 <typeAliases> <typeAlias type="com.tuling.entity.Employee" alias="employee"></typeAlias> </typeAliases>
   */
  private Class<?>[] typeAliases;

  /**
   * 若不想一一的去配置 <typeAlias></typeAlias>该节点, 那么通过该节点可以批量设置别名(别名的的名称就是类的名称 不区分大小写) <typeAliases>
  ...
  /**
   * 通过该开关来控制只支持开启哪种别名 factoryBean.setTypeAliasesPackage("com.tuling.entity");
